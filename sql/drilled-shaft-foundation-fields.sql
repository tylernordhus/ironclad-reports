alter table public.pour_log_foundations
  add column if not exists shaft_diameter text,
  add column if not exists anchor_bolt_projection text;
...
begin
  ...
  if exists (
    ...
    where (
      ...
      coalesce(trim(total_depth), '') <> '' or
      coalesce(trim(estimated_yards), '') <> '' or
      coalesce(trim(shaft_diameter), '') <> '' or
      coalesce(trim(anchor_bolt_projection), '') <> '' or
      coalesce(trim(notes), '') <> ''
    )
